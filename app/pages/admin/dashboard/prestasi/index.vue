...
const getAchievements = async () => {
    try {
        achievements.value =  await $api('/achievement', {
            method: 'GET'
        })
    } catch(err) {
        console.log(err)
            ...
            <div class="container-fluid p-4">
                ...
                <div class="card border-0 shadow-sm rounded-4">
                    <div class="card-body p-4">
                        ...
                        <NuxtLink to="/admin/dashboard/prestasi/add/" class="btn btn-primary mb-3">Tambah</NuxtLink>
                            ...
                            <table class="table table-hover align-middle">
                                ...
                                <tbody>
                                    <tr v-for="achievement in achievements" :key="achievement.id">
                                        ...
                                        <td>{{ potongTeks(achievement.description) }}</td>
                                        <td>{{ achievement.level }}</td>
                                        <td>{{ achievement.imageUrl }}</td>
                                        <td>{{ formatDate(achievement.createdAt) }}</td>
                                        <td>
                                            <button class="btn btn-sm btn-danger me-1" @click="deleteAchievement(achievement.id)">Delete</button>
                                            <!-- <button class="btn btn-sm btn-light border">Edit</button> -->
                                            <NuxtLink :to="`/admin/dashboard/prestasi/edit/${achievement.id}`" class="btn btn-sm btn-light">Edit</NuxtLink>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
