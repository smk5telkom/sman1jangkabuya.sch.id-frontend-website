...
const getAchievement = async (id) => {
    try {
        const data = await apiRequest(`/achievement/${id}`)
        console.log(data);
        achievements.value = {
            id: data.id,
            title: data.title,
...
const submitEdit = async (id) => {
    try {
        ...
        formData.append('title', achievements.value.title),
        formData.append('subtitle', achievements.value.subtitle),
        formData.append('description', achievements.value.description),
        formData.append('level', achievements.value.level);

        if(imageFile.value) {
            formData.append('image', imageFile.value)
        }

        await apiRequest(`/achievement/${id}`, {
            method: 'PATCH',
            body: formData
        });

        alert('berhasil')
        // getAchievement(achievements.value.id)
                    ...
                    <div class="card-body p-4">
                        ...
                        <form @submit.prevent="submitEdit(achievements.id)">
                            ...
                            <div class="mb-3">
                                ...
                                <input v-model="achievements.title" type="text" class="form-control" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Sub Judul</label>
                                <input v-model="achievements.subtitle" type="text" class="form-control" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Isi Konten</label>
                                <textarea v-model="achievements.description" class="form-control" rows="5" required></textarea>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Tingkat</label>
                                <select v-model="achievements.level" class="form-control">
