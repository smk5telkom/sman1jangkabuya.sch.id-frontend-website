    ...
    try {
        await apiRequest('/announcements', {
            ...
            body: {
                ...
                typeTitle: announcement.value.typeTitle,
                typeContent: announcement.value.typeContent,
            }
        });
                    ...
                    <div class="card-body p-4">
                        ...
                        <form @submit.prevent="addAnnouncement">
                            <div class="mb-3">
                                ...
                                <input v-model="announcement.title" type="text" class="form-control" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Isi Berita</label>
                                <textarea v-model="announcement.content" class="form-control" rows="5" required></textarea>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Pilih Tipe</label>
                                <select v-model="announcement.type" class="form-select" required>
                                    <option value="Link">Link</option>
                                    <option value="Button">Button</option>
                                </select>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">{{ announcement.type === 'Link' ? 'Judul LInk' : 'Judul Button' }}</label>
                                <input type="text" v-model="announcement.typeTitle" class="form-control" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">{{ announcement.type === 'Link' ? 'URL Link' : 'Isi Button' }}</label>
                                <input type="text" v-model="announcement.typeContent" :placeholder="announcement.type === 'Link' ? 'Isi dengan link tujuan' : 'Isi dengan link file'" class="form-control" required>
                            </div>
